...
namespace MaxSRP
{
    public class ScenePRTBaker : MonoBehaviour
    {
        public void BakePRT()
        {
            Material old_skybox = RenderSettings.skybox;
            Material cosine_skybox = new Material(Shader.Find("MaxSRP/CosineSkybox"));
            ...
            Shader black_shader = Shader.Find("MaxSRP/BlackShader");
            ...
            foreach (MeshRenderer mr in to_compute)
            {
                if (mr.gameObject.isStatic) //compute only static objects
                {
                    ...
                    Material mat = new Material(Shader.Find("MaxSRP/SH_Shader"));
